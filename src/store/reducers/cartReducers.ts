...
export const cartSplice = createSlice({
  ...
  reducers: {
    ...
    //   Remove base on position
      if (state.items[action.payload].amount == 1) state.items.splice(action.payload, 1)
      else state.items[action.payload].amount -= 1
...
export const { addItem, removeItem, pendingCart,newCart, updateItem, clearCart, checkoutCart } = cartSplice.actions
